...
int riempi_array_nodi(strint *root, strint *a[])
{
  if(root!=NULL) {
    ...
    a[messisx] = root;
    int messidx = riempi_array_nodi(root->right, &a[messisx+1]);
...
int conta_parole(strint *root , int len){ 
  ...
  int tot=0;
  if(root!=NULL) {
    tot += conta_parole(root->left, len)  ;
    tot += strlen(root->s)==len?root->n:0 ;
    tot += conta_parole(root->right , len);
  }
  return tot;
}
...
int max_length(strint *root){
    if(root == NULL)
        return -1 ;
    int lres = max_length(root->left) ;
    int cres = strlen(root->s);
    int rres = max_length(root->right);
    if (lres > cres)
        cres = lres;
    if (rres > cres)
        cres = rres;
...
int main(int argc, char *argv[])
{
  ...
  int max = max_length(root);  
  for(int i = 1 ; i < max ; i++){
    fprintf(stderr, "#parole lunghe %d : %d\n", i , conta_parole(root, i) );
  }
